    ...
    @foreach ($self_conf as $studi)
        @if ($studi['nama'] === 'studi kasus 1')
            ...
            <div class="my-4 -translate-x-8">
                ...
                <form action="{{ route('accurate-self-assesment.store') }}" class="space-y-5" id="form-studkas1_selfcon">
                    @foreach ($studi['pertanyaan'] as $key => $soal)
                        ...
                        @foreach ($soal['pilihan'] as $key2 => $pilihan)
                            <div class="flex items-center mt-3 rounded-md">
                                <input type="radio" id="{{ $key . '_' . $key2 . '_studkas1_selfcon' }}"
                                    name="{{ $key }}" value="{{ $key2 }}"
                                    {{ isset($show_jawaban) && $soal['jawaban_user'] == $key2 ? 'checked' : '' }}>
                                <label for="{{ $key . '_' . $key2 . '_studkas1_selfcon' }}" class="ml-2">
                                    {{ $key2 }}. {{ $pilihan }}
                                </label>
                            </div>
                        @endforeach
...
@push('custom-script')
    <script>
        $('#form-studkas1_selfcon').submit(function(e) {
            e.preventDefault();
            ...
            axios.post("{{ route('accurate-self-assesment.store') }}", {
                ...
                .then((response) => {
                    ...
                    $.each(response.data.jawaban_old, function(questionNumber, jawabanOld) {
                        ...
                        const oldAnswerId = `#${questionNumber}_${jawabanOld}_studkas1_selfcon`;
                        const correctAnswerId = `#${questionNumber}_${jawabanBenar}_studkas1_selfcon`;

                        // Define the corresponding label selectors using the 'for' attribute
                        const oldLabel = `label[for='${questionNumber}_${jawabanOld}_studkas1_selfcon']`;
                        const correctLabel = `label[for='${questionNumber}_${jawabanBenar}_studkas1_selfcon']`;
