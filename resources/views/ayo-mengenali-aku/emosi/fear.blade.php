    ...
    @foreach ($emosis as $keyz => $emosi)
        @if ($emosi['nama_emosi'] == 'Takut')
            ...
            <div class="my-4 -translate-x-8">
                ...
                <form action="{{ route('ayo-mengenali-aku.index') }}" class="space-y-5" id="form-fear">
                    @foreach ($emosi['pertanyaan'] as $key => $soal)
                        ...
                        @foreach ($soal['pilihan'] as $key2 => $pilihan)
                            <div class="flex items-center mt-3 rounded-md">
                                <input type="radio" id="{{ $key . '_' . $key2 . '_fear' }}"
                                    name="{{ $key }}" value="{{ $key2 }}"
                                    {{ isset($show_jawaban) && $soal['jawaban_user'] == $key2 ? 'checked' : '' }}>
                                <label for="{{ $key . '_' . $key2 . '_fear' }}" class="ml-2">
                                    {{ $key2 }}. {{ $pilihan }}
                                </label>
                            </div>
                        @endforeach
            ...
            axios.post("{{ route('ayo-mengenali-aku.store') }}", {
                ...
                .then((response) => {
                    ...
                    $.each(response.data.jawaban_old, function(questionNumber, jawabanOld) {
                        ...
                        const oldAnswerId = `#${questionNumber}_${jawabanOld}_fear`;
                        const correctAnswerId = `#${questionNumber}_${jawabanBenar}_fear`;

                        // Define the corresponding label selectors using the 'for' attribute
                        const oldLabel = `label[for='${questionNumber}_${jawabanOld}_fear']`;
                        const correctLabel = `label[for='${questionNumber}_${jawabanBenar}_fear']`;
