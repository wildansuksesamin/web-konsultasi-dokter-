        ...
        @if ($emosi['nama_emosi'] == 'Kesedihan')
            <table class="table-auto overflow-auto w-full border-separate border-spacing-y-3 border-spacing-x-4">
                <tbody class="">
                    ...
                    <tr>
                        <th>Reaksi terhadap Kesedihan:</th>
                        <td>{{ $emosi['reaksi'] }}</td>
                    </tr>
                    <tr>
                        <th>Dampak Kesedihan:</th>
                        <td>{{ $emosi['dampak'] }}</td>
                    </tr>
                    <tr>
            ...
            <div class="my-4 -translate-x-8">
                ...
                <form action="{{ route('ayo-mengenali-aku.index') }}" class="space-y-5" id="form-sadness">
                    @foreach ($emosi['pertanyaan'] as $key => $soal)
                        ...
                        @foreach ($soal['pilihan'] as $key2 => $pilihan)
                            <div class="flex items-center mt-3 rounded-md">
                                <input type="radio" id="{{ $key . '_' . $key2 . '_sadness' }}"
                                    name="{{ $key }}" value="{{ $key2 }}"
                                    {{ isset($show_jawaban) && $soal['jawaban_user'] == $key2 ? 'checked' : '' }}>
                                <label for="{{ $key . '_' . $key2 . '_sadness' }}" class="ml-2">
                                    {{ $key2 }}. {{ $pilihan }}
                                </label>
                            </div>
                        @endforeach
...
@push('custom-script')
    <script>
        $('#form-sadness').submit(function(e) {
            e.preventDefault();
            // remove all bg color
            ...
            axios.post("{{ route('ayo-mengenali-aku.store') }}", {
                ...
                .then((response) => {
                    ...
                    $.each(response.data.jawaban_old, function(questionNumber, jawabanOld) {
                        ...
                        const oldAnswerId = `#${questionNumber}_${jawabanOld}_sadness`;
                        const correctAnswerId = `#${questionNumber}_${jawabanBenar}_sadness`;

                        // Define the corresponding label selectors using the 'for' attribute
                        const oldLabel = `label[for='${questionNumber}_${jawabanOld}_sadness']`;
                        const correctLabel = `label[for='${questionNumber}_${jawabanBenar}_sadness']`;
