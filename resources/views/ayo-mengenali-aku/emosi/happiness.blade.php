<div>
    @foreach ($emosis as $keyz => $emosi)
        @if ($emosi['nama_emosi'] == 'Kebahagiaan')
            ...
            <div class="my-4 -translate-x-8">
                ...
                <form action="{{ route('ayo-mengenali-aku.index') }}" class="space-y-5" id="form-happiness">
                    @foreach ($emosi['pertanyaan'] as $key => $soal)
                        ...
                        @foreach ($soal['pilihan'] as $key2 => $pilihan)
                            <div class="flex items-center mt-3 rounded-md">
                                <input type="radio" id="{{ $key . '_' . $key2 . '_happiness' }}"
                                    name="{{ $key }}" value="{{ $key2 }}"
                                    {{ isset($show_jawaban) && $soal['jawaban_user'] == $key2 ? 'checked' : '' }}>
                                <label for="{{ $key . '_' . $key2 . '_happiness' }}" class="ml-2">
                                    {{ $key2 }}. {{ $pilihan }}
                                </label>
                            </div>
                        @endforeach
...
@push('custom-script')
    <script>
        $('#form-happiness').submit(function(e) {
            e.preventDefault();
            // remove all bg color
            ...
            axios.post("{{ route('ayo-mengenali-aku.store') }}", {
                ...
                .then((response) => {
                    ...
                    $.each(response.data.jawaban_old, function(questionNumber, jawabanOld) {
                        ...
                        const oldAnswerId = `#${questionNumber}_${jawabanOld}_happiness`;
                        const correctAnswerId = `#${questionNumber}_${jawabanBenar}_happiness`;

                        // Define the corresponding label selectors using the 'for' attribute
                        const oldLabel = `label[for='${questionNumber}_${jawabanOld}_happiness']`;
                        const correctLabel = `label[for='${questionNumber}_${jawabanBenar}_happiness']`;
