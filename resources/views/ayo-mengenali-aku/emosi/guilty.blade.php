<div>
    @foreach ($emosis as $keyz => $emosi)
        @if ($emosi['nama_emosi'] == 'Rasa Bersalah')
            <table class="table-auto overflow-auto w-full border-separate border-spacing-y-3 border-spacing-x-4">
                <tbody class="">
                    ...
                    <tr>
                        <th>Reaksi terhadap Bersalah:</th>
                        <td>{{ $emosi['reaksi'] }}</td>
                    </tr>
                    {{-- <tr>
                        <th>Dampak Bersalah:</th>
                        <td>{{ $emosi['dampak'] }}</td>
                    </tr> --}}
                    <tr>
            ...
            <div class="my-4 -translate-x-8">
                ...
                <form action="{{ route('ayo-mengenali-aku.index') }}" class="space-y-5" id="form-guilty">
            ...
            axios.post("{{ route('ayo-mengenali-aku.store') }}", {
                ...
                .then((response) => {
                    ...
                    $.each(response.data.jawaban_old, function(questionNumber, jawabanOld) {
                        ...
                        const oldAnswerId = `#${questionNumber}_${jawabanOld}_guilty`;
                        const correctAnswerId = `#${questionNumber}_${jawabanBenar}_guilty`;

                        // Define the corresponding label selectors using the 'for' attribute
                        const oldLabel = `label[for='${questionNumber}_${jawabanOld}_guilty']`;
                        const correctLabel = `label[for='${questionNumber}_${jawabanBenar}_guilty']`;
